...
public class DbBean {
 ...
 // Generates the HTML dropdown menu for the selection form
    ...
    // Generates the HTML dropdown menu with ONLY key values for deletion, submitting to the destination page
    public String getDeleteDropdownMenu(String actionPage) {
        StringBuilder sb = new StringBuilder("<form action='" + actionPage + "' method='POST'>");
        sb.append("<label for='id'>Select a Movie ID to Delete: </label>");
        sb.append("<select name='id' id='id'>");
        ...
        sb.append("</select> ");
        if (hasRecords) {
            sb.append("<input type='submit' name='deleteSubmit' value='Delete Record'>");
        } else {
            sb.append("<input type='submit' name='deleteSubmit' value='Delete Record' disabled>");
        }
        sb.append("</form>");
        return sb.toString();
    }
